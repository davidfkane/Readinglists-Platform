...
function logfile($input, $level='WHITE', $method='METHOD', $class='CLASS'){
	#USAGE:	 logfile("", 'model', __FUNCTION__, __FILE__);
	
	$myFile = "/var/www/signposts/debug.txt";
	$level = strtoupper($level);
	
	$bold = 1; // 1 or 0;
	$colours = array('MODEL' => 1, 'VIEW' => 2, 'CONTROLLER' => 3, 'BLACK' => 0,  'RED' => 1,  'GREEN' => 2,  'YELLOW' => 3,  'BLUE' => 4,  'MAGENTA' => 5,  'CYAN' => 6,  'WHITE' => 7 );
	 
	$write = "\033[0$bold;3".$colours['WHITE']."m$class\t => $method \033[0$bold;3$colours[$level]m";
	$write .= $input;
